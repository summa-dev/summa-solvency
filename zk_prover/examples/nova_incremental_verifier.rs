...
use serde_json::json;
use summa_solvency::merkle_sum_tree::{utils::fp_to_big_uint, MerkleSumTree};
...
// - [ ] Add further steps and assert that the correct values are returned
// - [ ] Fix Poseidon Hasher
// - [ ] Add Benchmark for Nova 
fn run_test(circuit_filepath: String, witness_gen_filepath: String) {
    ...
    let username = proof.entry.username_to_big_uint();
    let username = username.to_string();

    let user_balances = proof.entry.balances();
    let user_balances = user_balances
        .iter()
        .map(|balance| balance.to_string())
        .collect::<Vec<String>>();

    let path_element_hashes = proof.sibling_hashes;
    let path_element_hashes = path_element_hashes
        .iter()
        .map(|hash| fp_to_big_uint(hash.to_owned()).to_string())
        .collect::<Vec<String>>();

    let path_indices = proof.path_indices;
    let path_indices = path_indices
        .iter()
        .map(|index| fp_to_big_uint(index.to_owned()).to_string())
        .collect::<Vec<String>>();

    let path_element_balances = proof.sibling_sums;
    let path_element_balances = path_element_balances
        .iter()
        .map(|balances| {
            balances
                .iter()
                .map(|balance| fp_to_big_uint(balance.to_owned()).to_string())
                .collect::<Vec<String>>()
        })
    ...
    for _i in 0..iteration_count {
        ...
        private_input.insert("username".to_string(), json!(username));
        private_input.insert("user_balances".to_string(), json!(user_balances));
        private_input.insert(
            "path_element_hashes".to_string(),
            json!(path_element_hashes),
        );
        private_input.insert("path_indices".to_string(), json!(path_indices));
        private_input.insert(
            "path_element_balances".to_string(),
            json!(path_element_balances),
        );
