...
mod test {
    ...
    // TODO: fix deploy contract for tests with verifier contracts
    // #[tokio::test]
    ...
    #[tokio::test]
    async fn test_snapshot_generate_inclusion_proof() -> Result<(), Box<dyn Error>> {
        const K: u32 = 17;
        const N_CURRENCIES: usize = 2;
        const N_USERS: usize = 16;

        let ptau_path = "ptau/hermez-raw-17";

        // Evaluate polynomial
        let circuit = UnivariateGrandSum::<N_USERS, N_CURRENCIES>::init_empty();
        let (params, pk, vk) = generate_setup_artifacts(K, Some(ptau_path), circuit).unwrap();
        ...
        // from full_prover get advice_poly
        let mut snapshot = Snapshot::<N_CURRENCIES, N_USERS>::new(advice_polys, ptau_path).unwrap();

        let proof = snapshot.generate_proof_of_inclusion(0).unwrap();
    ...
    async fn test_round_features() -> Result<(), Box<dyn Error>> {
        ...
        // TODO: applied on V2 smart contract
