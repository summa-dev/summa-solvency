mod test {
    use std::{sync::Arc, time::Duration};

    use ethers::{
        abi::Address,
        prelude::{ContractFactory, SignerMiddleware},
        providers::{Http, Middleware, Provider, StreamExt},
        signers::{LocalWallet, Signer},
        types::{Bytes, U256},
        utils::{hex, Anvil},
    };
    use num_bigint::{BigInt, BigUint};

    use crate::{
        contracts::generated::{
            mock_erc20::{self, MockERC20, MOCKERC20_ABI, MOCKERC20_BYTECODE},
            summa_contract::{ProofOfSolvencySubmittedFilter, Summa},
            verifier::SolvencyVerifier,
        },
        signer::signer::SummaSigner,
    };

    #[tokio::test]
    async fn test_signer_invalid_key() {
        const dummy_address_string: &str = "0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512";
        let address: Address = dummy_address_string.parse().unwrap();

        let result = std::panic::catch_unwind(|| {
            SummaSigner::initialize(
                vec!["wrong_key"],
                "0xde9be858da4a475276426320d5e9262ecfc3ba460bfac56360bfa6c4c28b4ee0",
                31337,
                "http://localhost:8545",
                address,
            )
        });
        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_sign_message() {
        let anvil = Anvil::new().spawn();

        let wallet: LocalWallet = anvil.keys()[0].clone().into();

        let provider = Provider::<Http>::try_from(anvil.endpoint())
            .unwrap()
            .interval(Duration::from_millis(10u64));

        let client = Arc::new(SignerMiddleware::new(
            provider,
            wallet.with_chain_id(anvil.chain_id()),
        ));

        let client_clone = Arc::clone(&client);

        let mock_erc20 = MockERC20::deploy::<()>(client, ())
            .unwrap()
            .send()
            .await
            .unwrap();

        let summa_contract = Summa::deploy(client_clone, mock_erc20.address())
            .unwrap()
            .send()
            .await
            .unwrap();

        let signer = SummaSigner::initialize(
            //Account #1
            vec!["0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d"],
            "0xde9be858da4a475276426320d5e9262ecfc3ba460bfac56360bfa6c4c28b4ee0",
            31337,
            anvil.endpoint().as_str(),
            summa_contract.address(),
        )
        .unwrap();

        let signatures = signer.generate_signatures("CryptoExchange").await.unwrap();
        assert_eq!(signatures.len(), 1);
        //Signature produced by the account #1
        assert_eq!(signatures[0].to_string(), "ea576c302228671c074bdf26fbd757d9c64016aa9974eaeb911274d1458a49f05aa5d4b9df5c0a4a68d2256bea8a6c762130e538b41f47fd54b2cbbdccd6a9de1c");
        drop(anvil);
    }

    #[tokio::test]
    async fn test_submit_proof_of_solvency() {
        let anvil: ethers::utils::AnvilInstance = Anvil::new()
            .mnemonic("test test test test test test test test test test test junk")
            .spawn();

        let cex_addr_1 = anvil.addresses()[1];
        let cex_addr_2 = anvil.addresses()[2];

        let wallet: LocalWallet = anvil.keys()[0].clone().into();

        let provider = Provider::<Http>::try_from(anvil.endpoint())
            .unwrap()
            .interval(Duration::from_millis(10u64));

        let client = Arc::new(SignerMiddleware::new(
            provider,
            wallet.with_chain_id(anvil.chain_id()),
        ));

        let client_clone = Arc::clone(&client);

        let factory = ContractFactory::new(
            MOCKERC20_ABI.to_owned(),
            MOCKERC20_BYTECODE.to_owned(),
            client.clone(),
        );

        let mock_erc20_deployment = factory.deploy(()).unwrap().send().await.unwrap();

        println!(
            "Deployed mock erc20 at {:?}",
            mock_erc20_deployment.address()
        );
        let mock_erc20 = MockERC20::new(mock_erc20_deployment.address(), client.clone());

        let verifer_contract = SolvencyVerifier::deploy(client_clone, ())
            .unwrap()
            .send()
            .await
            .unwrap();

        let summa_contract = Summa::deploy(Arc::clone(&client), verifer_contract.address())
            .unwrap()
            .send()
            .await
            .unwrap();

        let whole_amount: u64 = 556863;
        let decimals = mock_erc20.decimals().call().await.unwrap();
        let decimal_amount = U256::from(whole_amount) * U256::exp10(decimals as usize);
        println!("Decimal amount: {:?}", decimal_amount);
        println!("Address 2: {:?}", cex_addr_2);
        let mint_result = mock_erc20.mint(cex_addr_2, decimal_amount).await;
        println!("Mint result: {:?}", mint_result.is_ok());
        let erc20_balance = mock_erc20
            .balance_of(
                "0x3c44cdddb6a900fa2b585dd299e03d12fa4293bc"
                    .parse()
                    .unwrap(),
            )
            .await
            .unwrap();
        println!("ERC20 balance: {:?}", erc20_balance);

        let summaSigner = SummaSigner::initialize(
            vec![
                "0xdf57089febbacf7ba0bc227dafbffa9fc08a93fdc68e1e42411a14efcf23656e",
                "0xde9be858da4a475276426320d5e9262ecfc3ba460bfac56360bfa6c4c28b4ee0",
            ],
            "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80",
            anvil.chain_id(),
            anvil.endpoint().as_str(),
            summa_contract.address(),
        )
        .unwrap();
        summaSigner.submitProofOfSolvency(             "CryptoExchange".to_string(),
                    vec![cex_addr_1, cex_addr_2],
                    vec![
         ("0xea576c302228671c074bdf26fbd757d9c64016aa9974eaeb911274d1458a49f05aa5d4b9df5c0a4a68d2256bea8a6c762130e538b41f47fd54b2cbbdccd6a9de1c").parse().unwrap(),
          ("0x5dc26d1b8cf94a11897872bbba7301fcf9e16a9767b4170b978586659b9ec52842621d323f42bf398ae23970fa21194ba8319faea4b2fd85e133a112702e21ab1b").parse().unwrap(),
          ],
                    vec![mock_erc20.address()],
                   vec![U256::from(556863),U256::from(556863)],
                  serde_json::from_str::<U256>("\"0x2E021D9BF99C5BD7267488B6A7A5CF5F7D00222A41B6A9B971899C44089E0C5\"").unwrap(),
                   ( "")
                   .parse().unwrap()
        ).await;

        let logs = summa_contract
            .proof_of_solvency_submitted_filter()
            .query()
            .await
            .unwrap();

        assert_eq!(logs.len(), 1);

        drop(anvil);
    }
}
