...
contract Verifier {
    ...
    uint256 internal constant SNARK_PROOF_LEN_CPTR = 0x144;
    uint256 internal constant     SNARK_PROOF_CPTR = 0x164;
    ...
    function verifyProof(
        ...
        assembly {
...

            {
                ...
                success := and(success, eq(0x15e0, calldataload(PROOF_LEN_CPTR)))
                if iszero(success) {
                    mstore(0, "Invalid proof length")
                    revert(0, 0x20)
                }
                // Check valid length of snark proof
                success := and(success, eq(0x1500, calldataload(SNARK_PROOF_LEN_CPTR)))
                if iszero(success) {
